...
export class VoucherComponent implements OnInit, OnDestroy {
  ...
  calculateTotal(formName) {
    ...
    if (
      this[formName].controls.GAMOUNT_DISC_TYPE.value == 1 &&
      this[formName].controls.DISC_G_PERCENT.value != ""
    ) {
      this[formName].controls["DISC_GAMOUNT"].setValue(
        (
          (this.total_amount * this[formName].controls.DISC_G_PERCENT.value) /
          100
        ).toFixed(2)
      );
    }
    if (
      this[formName].controls.TAX_TYPE.value == 1 &&
      this[formName].controls.TAX_PERCENT.value != ""
    ) {
      this[formName].controls["TAX_GAMOUNT"].setValue(
        (
          ((this.total_amount - this[formName].controls["DISC_GAMOUNT"].value) *
            this[formName].controls.TAX_PERCENT.value) /
          100
        ).toFixed(2)
      );
    ...
    if (
      this[formName].controls.SHIP_TYPE.value == 1 &&
      this[formName].controls.SHIP_PERCENT.value != ""
    ) {
      this[formName].controls["SHIP_GAMOUNT"].setValue(
        (
          ((this.total_amount - this[formName].controls["DISC_GAMOUNT"].value) *
            this[formName].controls.SHIP_PERCENT.value) /
          100
    ...
    this[formName].controls["FINAL_AMOUNT"].setValue(
      (
        this.total_amount -
        this[formName].controls["DISC_GAMOUNT"].value -
        this[formName].controls["TAX_GAMOUNT"].value -
        this[formName].controls["SHIP_GAMOUNT"].value
      ).toFixed(2)
    );
    this[formName].controls["BALANCE"].setValue(
      (
        this[formName].controls["FINAL_AMOUNT"].value -
        this[formName].controls["PAID_AMOUNT"].value
      ).toFixed(2)
    );
  }
          ...
          if (res["data"]["getVoucher_Q"].CODE !== "") {
            if (getOneDoc) {
              ...
              for (let i = 0; i < this.data["ADDRESS"].length; i++) {
                this.ADDRESSES("editDataForm").push(
                  this._fb.group(this.data["ADDRESS"][i])
                );
              }
              for (let i = 0; i < this.data["PHONE"].length; i++) {
                this.PHONES("editDataForm").push(
                  this._fb.group(this.data["PHONE"][i])
                );
              }
              for (let i = 0; i < this.data["EMAILID"].length; i++) {
                this.EMAILIDS("editDataForm").push(
                  this._fb.group(this.data["EMAILID"][i])
                );
              }
              for (let i = 0; i < this.data["LINE"].length; i++) {
                this.LINES("editDataForm").push(
                  this._fb.group(this.data["LINE"][i])
                );
              }
