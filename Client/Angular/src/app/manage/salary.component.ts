...
export class SalaryComponent implements OnInit, OnDestroy {
  ...
  getEmployee(id) {
    ...
    return (this.querySubscription = this._backendService
      ...
      .subscribe(
        (res) => {
          this.addDataForm.controls["emplid"].patchValue(
            res["data"]["getEmployee_Q"][0]._id
          );
          this.addDataForm.controls["emplskey"].patchValue(
            res["data"]["getEmployee_Q"][0].SKEY
          );
          this.addDataForm.controls["empldescr"].patchValue(
            res["data"]["getEmployee_Q"][0].DESCR
          );
          this.addDataForm.controls["empllastname"].patchValue(
            res["data"]["getEmployee_Q"][0].LAST_NAME
          );
          this.addDataForm.controls["emplsalcode"].patchValue(
            res["data"]["getEmployee_Q"][0].SALARY_CODE
          );

          this._backendService
            .getSalaryCode("", res["data"]["getEmployee_Q"][0].SALARY_CODE)
            .subscribe((res) => {
